...
MVP에서는 `instance` label을 정확히 일치시켜 현재 서버의 node_exporter series만 조회한다.
Grafana 변수인 `$node`, `$interval`을 query에 그대로 전달하지 않고 설정의 `instance`, `rate_interval` 값으로 치환한다.
로그와 같은 start/end 범위를 사용하며, CPU rate 계산은 각 평가 시각 직전의 `rate_interval` sample을 참조할 수 있다.
...
100 - (
  avg by (instance) (
    rate(node_cpu_seconds_total{instance="TARGET_HOST:9100",mode="idle"}[5m])
  ) * 100
)
```
...
100 * (
  1 -
  (
    node_memory_MemAvailable_bytes{instance="TARGET_HOST:9100"}
    /
    node_memory_MemTotal_bytes{instance="TARGET_HOST:9100"}
  )
)
```
...
```promql
node_load1{instance="TARGET_HOST:9100"}
```

현재 MVP의 기본 query는 CPU 사용률, memory 사용률, load average 1분 세 가지로 제한한다.
아래 filesystem, disk, network, pressure query는 후속 확장 후보이다.
...
개별 query 실패 또는 빈 결과는 성공한 query를 버리지 않고 manifest에 `partial`, `failed`, `skipped`로 기록한다.
...
현재 Prometheus HTTP 수집은 Python 표준 라이브러리를 사용한다.
...
### Phase 3: Prometheus MVP Baseline
