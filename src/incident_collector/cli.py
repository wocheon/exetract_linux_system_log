...
from .collectors import collect_files, collect_journal, collect_prometheus, collect_system_summary
...
def _run_collection(config: CollectorConfig, time_range: TimeRange) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="incident-collector-", dir=temp_parent) as temporary:
        ...
        results: list[CollectionResult] = [
            collect_system_summary(os_info, staging),
            collect_prometheus(config.prometheus, time_range, staging),
        ]
